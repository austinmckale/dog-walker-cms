...
type Service = 'walk' | 'transport'

export default function SchedulePage() {
  const params = useSearchParams()
  const initialService = (params.get('service') === 'transport' ? 'transport' : 'walk') as Service
  ...
  function onChange(e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) {
    ...
    if (name === 'service') setService(value as Service)
      ...
      <main className="mx-auto max-w-3xl px-4 py-8 space-y-6">
        ...
        {embedUrl ? (
          ...
          <form onSubmit={onSubmit} className="space-y-4 bg-white p-4 rounded-xl shadow">
            ...
            {status && (
              <p className={`text-sm mt-2 ${status.ok ? 'text-green-600' : 'text-red-600'}`}>{status.msg}</p>
            )}
          </form>
        )}
